...
def BuildModel(input_shape=(227,227,10,1)):
    ...
    #Spatial Encoder
    #Conv3D is used to create kernel: filters (dimensionality of output space), size (tuple specifying the depth, height and width)
    #strides(list of 3 integers specifying the strides), padding(one of valid or same), activation function 
    ...
    #Temporal Encoder
    #dropout(float between 0 to 1 o drop the linear transformation)
    #recurrent_dropout(float between 0 to 1 o drop the recurrent linear transformation)
...
def ShowVideo(cap, v_frame, text):
    ...
    cv2.imshow('Anomaly Detection',v_frame)
